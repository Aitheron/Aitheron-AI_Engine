...
def vep_call(vcf_like):
    ...
    resp = requests.post(VEP_URL,headers=HEADERS_JSON,data=json.dumps(payload),timeout=180)
...
def _apply_type_flags(type_value: str) -> dict: # Refatorar posteriormente (função já existe)
    flags = {c: 0 for c in TYPE_FLAG_COLS}
    norm = (type_value or "").strip().lower()
    if norm in {"snv","single nucleotide variant","substituição","substituicao"}:
        flags["IsSNV"] = 1
    elif norm in {"deletion","deleção","deleçao","delecao"}:
        flags["IsDeletion"] = 1
    elif norm in {"duplication","duplicação","duplicacao"}:
        flags["IsDuplication"] = 1
    elif norm in {"insertion","inserção","insercao"}:
        flags["IsInsertion"] = 1
    elif norm in {"indel"}:
        flags["IsIndel"] = 1
    return flags
...
def enrich_with_vep_to_csv(in_csv, out_csv):
    ...
    for i, row in df.iterrows():
        ...
        type_flags = _apply_type_flags(str(row.get("Type","")))

        rec_out=dict(row)
        rec_out.update({
            "ConsequenceTerms":most,
            "impact":impact_str,
            "ImpactRank": impact_num,
            "IsCoding": is_coding,

...
from ml.train.metrics import compute_binary_metrics, pick_best_threshold_by_f1
...
def train_one_fold(cfg: Config, fold_idx: int, X_train, y_train, g_train, X_val, y_val, g_val, input_dim: int, pre: Preprocessor, outdir_fold: str) -> dict:
    ...
    criterion0 = nn.BCEWithLogitsLoss(pos_weight=pw0)
    criterion1 = nn.BCEWithLogitsLoss(pos_weight=pw1)
    ...
    # masks para heads
    mask_tr = make_mask_for_heads(pd.Series(g_train))
    mask_va = make_mask_for_heads(pd.Series(g_val))
    m0_tr = torch.tensor(mask_tr["BRCA1"], dtype=torch.float32, device=device).unsqueeze(1)
    m1_tr = torch.tensor(mask_tr["BRCA2"], dtype=torch.float32, device=device).unsqueeze(1)
    ...
    for epoch in range(cfg.train.epochs):
        ...
        for xb, yb in dl_train:
            ...
            # aplica loss apenas nos exemplos da cabeça correspondente
            loss0 = criterion0(logits0[m0_tr[:len(xb)] > 0.5], yb[m0_tr[:len(xb)] > 0.5]) if (m0_tr[:len(xb)].sum() > 0) else 0.0
            loss1 = criterion1(logits1[m1_tr[:len(xb)] > 0.5], yb[m1_tr[:len(xb)] > 0.5]) if (m1_tr[:len(xb)].sum() > 0) else 0.0
            loss = 0.0
            if isinstance(loss0, torch.Tensor):
                loss = loss + loss0
        ...
        # thresholds por cabeça via F1 no val (apenas onde a cabeça se aplica)
        thr0 = pick_best_threshold_by_f1(y_val[m0_va_np > 0.5], prob0[m0_va_np > 0.5]) if (m0_va_np > 0.5).any() else 0.5
